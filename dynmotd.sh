...
MAINLOG="$USER_HOME/.dynmotd/maintenance.log"
ENVFILE="$USER_HOME/.dynmotd/environment.cfg"
...
function show_user_info() {
	...
	if [ "$USER_INFO" = "1" ]; then
		...
		SUPERUSERCOUNT=$(cat $USER_HOME/.ssh/authorized_keys | grep -E '^ssh-' | wc -l)
		...
		SUPERUSER=$(echo $(
			readarray -t rows <$USER_HOME/.ssh/authorized_keys
			for row in "${rows[@]}"; do
				row_array=(${row})
				third=${row_array[2]}
				if [ -z $third ]; then echo -n "- Unknown -, "; else echo -n "${third}, "; fi
			done
		) | sed '1,$s/\([^,]*,[^,]*,[^,]*,\)/\1\n\\033[1;32m\t          /g' | sed 's/,$//')
