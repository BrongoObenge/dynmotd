...
MAINLOG="/home/$USER/.dynmotd/maintenance.log"
ENVFILE="/home/$USER/.dynmotd/environment.cfg"
...
function show_user_info () {
    ...
    if [ "$USER_INFO" = "1" ]; then
        ...
        SYSTEMUSERCOUNT=$(cat /etc/passwd |grep -E ':x:12[0-9][0-9]' |grep ':/bin/bash|:/bin/zsh|:/bin/sh' |wc -l)

        ## who is a system user, only check uid <1000 and has a login shell
	SYSTEMUSER=$(cat /etc/passwd | grep -E ':x:10[0-9][0-9]' | grep -E '/bin/bash|/bin/sh|/bin/zsh' | awk '{if ($0) print}' | awk -F ':' {'print $1'} | awk -vq=" " 'BEGIN{printf""}{printf(NR>1?",":"")q$0q}END{print""}' | cut -c2- | sed 's/ ,/,/g' | sed '1,$s/\([^,]*,[^,]*,[^,]*,[^,]*,[^,]*,\)/\1\n\\033[1;32m\t          /g')

        ## how many ssh super user (root) are there
        SUPERUSERCOUNT=$(cat /home/$USER/.ssh/authorized_keys |grep -E '^ssh-' |wc -l)
        ...
        SUPERUSER=$(echo $(readarray -t rows < /home/$USER/.ssh/authorized_keys; for row in "${rows[@]}"; do row_array=(${row}); third=${row_array[2]}; if [ -z $third ]; then echo -n "- Unknown -, "; else echo -n "${third}, "; fi; done) |sed '1,$s/\([^,]*,[^,]*,[^,]*,\)/\1\n\\033[1;32m\t          /g' |sed 's/,$//' )
